...
class Alliance
{
    ...
    public function run(array $post, $id = '', $key=''): void
    {
        ...
        if ($id && $key) {
            ...
            $this->database
                ->prepare("INSERT INTO user_alliance (user, alliance) VALUES (:user, :alliance)")
                ->execute([
                    ':user' => $_SESSION['id'],
                    ':alliance' => $alliance
                ]);
        ...
        if ($id) {
            ...
            $stmt = $this->database->prepare("SELECT user_alliance.*, users.name as discord, users.discriminator FROM user_alliance INNER JOIN users ON users.aid=user_alliance.user WHERE alliance=:alliance");
