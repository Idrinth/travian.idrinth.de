...
class Map
{
    ...
    public function run($post, $world)
    {
        ...
        $data['villages'] = array_map(
            function(array $row) {return ['x' => intval($row['x'], 10), 'y' => intval($row['y'], 10), 'alliance' => $row['alliance_name'], 'village' => $row['village_name'], 'player' => $row['player_name']];},
            $this->database->query("SELECT x,y,village_name,player_name,alliance_name FROM `$world`")->fetchAll(PDO::FETCH_ASSOC)
        );
